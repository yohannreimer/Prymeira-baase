...
async function createOrReuseTask(client:OperationalClient,input:Omit<TaskOccurrence,"id"|"createdAt"|"updatedAt">){
  ...
  let parentId:string|null=null;if(input.origin==="routine"&&input.routineId){parentId=generatedId("routine_occurrence");const parent=await client.query<{id:string}>(`INSERT INTO routine_occurrences (id,workspace_id,routine_id,due_date,audience_key,area_name_snapshot,routine_title_snapshot,routine_updated_at_snapshot) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (workspace_id,routine_id,due_date,audience_key) DO UPDATE SET id=routine_occurrences.id RETURNING id`,[parentId,input.workspaceId,input.routineId,input.dueDate,audienceKey,areaName,routineTitle,routineRevision]);parentId=parent.rows[0]!.id;}
