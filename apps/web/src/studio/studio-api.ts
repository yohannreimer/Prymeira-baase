...
export function mapStudioRitualSession(raw: RawStudioRitualSession): StudioRitualSession {
  return {
    ...
    status: raw.status,
    revision: raw.revision,
    answerRevision: raw.answer_revision ?? raw.answerRevision ?? raw.revision,
    supportMode: raw.support_mode ?? raw.supportMode ?? "guided_reflection",
    occurrenceAt: raw.occurrence_at ?? raw.occurrenceAt ?? required(raw.created_at, raw.createdAt, "created_at"),
